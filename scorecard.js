...
function extractData(body) {
    ...
    //  0 index -> team 1 name , 1-> team 2 name 
    ...
    // -> array 0 indec-> team 1 batting , 1 index-> team 2 bowling,
    ...
    //  fs.writeFileSync("bothTeam.html", htmlString);
...
function processTeam(teamElement, currTeam, opponentTeam, result, otherDetails){
    ...
    for (let i = 0; i < allRowswithextras.length; i++) {
        ...
        if (cols.length == 8) {
            ...
            // );
            let dataObj = {
                name, runs, balls, fours, sixes, opponentTeam, result, otherDetails 
            }
        
            dataOrganizer(currTeam, name, dataObj); // to save data into file or folder 
